...
#
#     /opt/devcontainer/fetch-zephyr.sh v4.3.0 0.17.4   # then: use-vanilla v4.3.0 0.17.4
#
# An omitted argument falls back to what containerEnv configures (see
# setup-sdks.sh); an explicitly empty one ("") skips that half.
#
...
# Same containerEnv-only config as setup-sdks.sh: the two versions come from the
# two paths, the toolchain and blob lists from their own vars.
: "${ZSDK_TOOLCHAINS:?not set -- it belongs in containerEnv in devcontainer.json}"
ZEPHYR_VER="${1-$(basename "$(dirname "${ZEPHYR_BASE:-/none/none}")")}"
SDK_VER="${2-$(basename "${ZEPHYR_SDK_INSTALL_DIR:-zephyr-sdk-}" | sed 's/^zephyr-sdk-//')}"
